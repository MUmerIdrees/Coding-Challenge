...
const async = require('async'); // Import async.js library
const app = express(); 
...
function fetchTitles(addresses, callback) {
    ...
    // Use async.eachSeries to iterate over addresses asynchronously
    async.eachSeries(addresses, (address, eachCallback) => {
      ...
      https.get(address, { maxRedirects: 5 }, response => {
        ...
        response.on('end', () => {
          ...
          eachCallback(); // Call the callback to move to the next iteration
        });
      }).on('error', error => {
